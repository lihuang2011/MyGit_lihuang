...
//		try{
			...
			String token = 
					Md5.saltMd5(userAgent+now);
			Cookie cookie = 
					new Cookie("token",now+"|"+token);
			//cookie 有路径问题 子目录的cookie可以发送给子目录
			//父目录下的cookie可以发给父目录及子目录
			cookie.setPath("/");
...
//			return new JsonResult<User>(msg);
//		}
	}
...
//		}
	...
	@ResponseBody
	public byte[] code(HttpServletRequest req) throws Exception{
//		try {
			byte[] buf;
			BufferedImage  img =
			...
			Graphics2D g=img.createGraphics();//拿到画笔
			int rgb = (int)(Math.random()*0xffffff);
			...
			req.getSession().setAttribute(
					"serverCode", code);
			g.drawString(code, x+3, 28-y);
			//绘制5条乱线
			for(int i=0;i<5;i++){
				int x1 = (int)(Math.random()*80);
				int x2 = (int)(Math.random()*80);
				int y1 = (int)(Math.random()*80);
				int y2 = (int)(Math.random()*80);
			...
			out.close();
			buf = out.toByteArray();//压缩img  成png byte数组
			return buf;
//		} catch (IOException e) {
//			
//			e.printStackTrace();
...
//		}
	...
	private String randomCode(){
		String chs = 
				"345678ABCDEFHJKLXYabcdrhp";
		char[] code = new char[4];
		for(int i=0;i<4;i++){
			int index = 
					(int)(Math.random()*chs.length());
	...
	public JsonResult<Boolean> 
		...
		String serverCode = 
				...
				.getAttribute("serverCode");
		if(code== null || 
				code.trim().isEmpty()){
			return new JsonResult<Boolean>("验证码不能为空！");
		}
		if(serverCode==null){
